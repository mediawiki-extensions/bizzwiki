...
/*
	...
	History:
	========
	1) modified AjaxSearch for inclusion of per-namespace 'search' right.
	2) modified AjaxWatch  for inclusion of per-namespace 'watch/unwatch' right.
*/
/** 
 * @package MediaWiki
 * @addtogroup Ajax
...
// }} BIZZWIKI END
	...
	while ( ( $row = $db->fetchObject( $res ) ) && ( ++$i <= $limit ) ) {
		...
		// BIZZWIKI {{
		$ns = $nt->getNamespace();
		if ( !$wgUser->isAllowed('search', $ns ) ) continue; //BIZZWIKI
		// }} BIZZWIKI
...
function wfAjaxWatch($pagename = "", $watch = "") {
	...
	if($watch) {
		if(!$watching && $wgUser->isAllowed('watch', $ns) /*BIZZWIKI*/) {
		...
		if($watching && $wgUser->isAllowed('unwatch', $ns) /*BIZZWIKI*/) {
			$dbw = wfGetDB(DB_MASTER);
			$dbw->begin();
			$article->doUnwatch();
			$dbw->commit();
