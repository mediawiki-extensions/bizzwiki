...
define('DPL2_VERSION', '1.2.5');               // current version 
 	...
 	if ( count($aTitleMatch)>0 ) {
		...
		foreach ($aTitleMatch as $link) {
			...
			if ($acceptOpenReferences)
    	    	...
    	    	$sSqlWhere .= 'page_title' . $sTitleMatchMode .  $dbr->addQuotes($link) ;
			$n++;
		}
		$sSqlWhere .= ')'; 
 	}
 	...
 	if ( count($aNotTitleMatch)>0 ) {
		...
		foreach ($aNotTitleMatch as $link) {
			...
			if ($acceptOpenReferences)
	        	...
	        	$sSqlWhere .= 'page_title' . $sNotTitleMatchMode . $dbr->addQuotes($link);
			$n++;
		}
		$sSqlWhere .= ')'; 
 	}
...
// ###### SHOW OUTPUT ######
	...
	$dpl = new DPL2($aHeadings, $bHeadingCount, $iColumns, $iRows, $iRowSize, $sRowColFormat, $aArticles, 
					...
					$aSecLabels, $aSecLabelsMatch, $aSecLabelsNotMatch, $parser, $logger, $aReplaceInTitle, $iTitleMaxLen);
	$dpl2result = $dpl->getText();
	if ($sResultsHeader != '')	$output .= str_replace('%PAGES%',$dpl->getRowCount(),$sResultsHeader) . "\n";

	return $output . $dpl2result;
}
...
class DPL2 {
	...
	var $mOutput;
	var $mReplaceInTitle;
 	var $filteredCount;	// number of (filtered) row count
	
	function DPL2($headings, $bHeadingCount, $iColumns, $iRows, $iRowSize, $sRowColFormat, $articles, $headingtype, $hlistmode, 
				  $listmode, $bescapelinks, $includepage, $includemaxlen, $includeseclabels, $includeseclabelsmatch, $includeseclabelsnotmatch, &$parser, $logger, $replaceInTitle, $iTitleMaxLen) {
		...
		if(!empty($headings)) {
			...
			foreach($headings as $heading => $headingCount) {
				...
				$this->mOutput .= $this->formatList($headingStart, $headingCount, $iTitleMaxLen);
						...
						if($sSecLabel[0] == '#') {
							...
							if ($mustMatch!='' || $mustNotMatch!='') {
								...
								foreach($secPiecesTmp as $nr => $onePiece ) {
									if (($mustMatch    !='' && !ereg($mustMatch   ,$onePiece)) ||
									    ($mustNotMatch !='' &&  ereg($mustNotMatch,$onePiece)) ) {
							...
							// Uses wfDplLstIncludetemplate() from LabeledSectionTransclusion extension to include templates from the page
 							...
 							$secPieces = wfDplLstIncludeTemplate($this->mParser, $article->mTitle->getPrefixedText(), $template1, $template2, $template2.'.default',$mustMatch,$mustNotMatch);
 							$secPiece[$s] = implode(isset($mode->aMultiSecSeparators[$s])? 
 								$this->substTagParm($mode->aMultiSecSeparators[$s], $pagename, $article, $this->filteredCount, $iTitleMaxLen):'',$secPieces);
 							if ($mode->iDominantSection>=0 && $s==$mode->iDominantSection && count($secPieces)>1)	$dominantPieces=$secPieces;
							...
							if ( ($mustMatch    !='' && !ereg($mustMatch   ,$secPiece[$s])) ||
								 ($mustNotMatch !='' &&  ereg($mustNotMatch,$secPiece[$s])) ) {
								$matchFailed=true;
								break;
							}
						}
